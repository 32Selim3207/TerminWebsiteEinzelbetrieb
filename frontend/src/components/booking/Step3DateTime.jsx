...
import {
  DOW_HEADER, MONTHS_DE, toDateStr, todayStr,
} from '../../constants/booking';

export default function Step3DateTime({ data, update }) {
  const today = useMemo(() => {
    const d = new Date();
    d.setHours(0, 0, 0, 0);
    return d;
  ...
  // Werkstatt profilinden blockedDates çek (public endpoint)
  useEffect(() => {
    api.get(`/werkstaette/${WERKSTATT_ID}`)
      .then((res) => setBlockedDates(res.data.blockedDates || []))
      .catch(() => {}); // Sessizce geç, kritik değil
  }, []);

  // Tarih seçilince backend'den boş saatleri çek
  useEffect(() => {
  ...
  // Takvim gridini hesapla
  const cells = useMemo(() => {
    const firstDay = new Date(calYear, calMonth, 1);
    const daysInMonth = new Date(calYear, calMonth + 1, 0).getDate();
    // getDay(): 0=Pazar, 1=Pazartesi... Biz Pazartesi-başlangıç istiyoruz
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      arr.push({
        ...
        dateStr,
        key: dateStr,
        isDisabled: isPast || isSunday || isBlocked,
  ...
  const handleDayClick = (cell) => {
    ...
    // Yeni tarih seçilince eski saati temizle
    ...
    <div>
      ...
      <div className="calendar">
        ...
        <div className="cal-grid">
          ...
          {cells.map((c) => (
            c.empty ? (
              <div key={c.key} className="cal-day is-empty" />
            ) : (
              <button
                key={c.key}
                type="button"
      ...
      {data.date && (
        <>
          ...
          {slotsLoading && (
            <div className="slots-empty">Zeiten werden geladen...</div>
          )}

          {!slotsLoading && slotsError && (
            <div className="field-error">{slotsError}</div>
          )}
            ...
            <div className="slot-grid">
              {slots.map((time) => (
                <button
                  ...
                  type="button"
                  className={
                    'slot-btn' + (data.startTime === time ? ' is-selected' : '')
                  }
