...
import { getServiceById, todayStr, MONTHS_DE, DAYS_DE } from '../../constants/booking';
...
export default function AppointmentsPanel({ appointments, onRefresh }) {
...
function AppointmentRow({ appointment: a, isOpen, onToggle, isLoading, onComplete, onCancel }) {
  const service = getServiceById(a.serviceType);
  return (
    <div className={'appt-row' + (isOpen ? ' is-open' : '')}>
      <button type="button" className="appt-row-summary" onClick={onToggle}>
        ...
        <span className="cell-service">{service?.label || a.serviceType}</span>
